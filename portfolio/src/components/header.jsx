      ...
      <div className="  flex justify-between items-center px-6 md:px-10 py-1">
        <div className="flex gap-4 items-center ">
          ...
          <h1 className="logo text-2xl font-bold tracking-widest text-cyan-400 ">
            Kunal<span className="text-purple-500">Sharma</span>
          </h1>
        </div>

        <div
          className={`${showNav ? "flex" : "hidden"} flex-col min-h-screen w-[300px] fixed top-0 right-0 bg-gradient-to-b from-cyan-950 to-black text-3xl gap-20 p-8`}
        >
          <div
            className="inset-0 h-screen w-screen fixed bg-black/60 "
            onClick={() => setShowNav(false)}
          ></div>
          <IoClose
            className="text-white text-5xl self-end cursor-pointer "
            onClick={() => setShowNav(false)}
          />
